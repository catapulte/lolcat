...
void loop() {
  ...
  String metaData = String(getBatteryStatus());
  String dhtData = "20|51";//getDHTData();
...
// #1|4810.8477N|126.8529W|29/1/2017@21:11:42.0|26.50|29.80|4.07#
    ...
    if (GPS.newNMEAreceived()) {
      if (!GPS.parse(GPS.lastNMEA()))
        ...
        if (GPS.fix)
          ...
          dataStr = String(GPS.day, DEC) + String('/')
                    + String(GPS.month, DEC)
                    + String("/20") + String(GPS.year, DEC)
                    + "@"
                    + String(GPS.hour, DEC) + String(':')
